...
import AudioSettings from './setting/AudioSettings';
const Settings = ({
  onClose,
  playerNames,
  setPlayerNames,
  ...
  const handleSave = () => {
    ...
    setSelectedDice(temporarySelectedDice);
    localStorage.setItem('selectedDice', temporarySelectedDice);
    ...
    setSounds(temporarySounds);
    Object.keys(temporarySounds).forEach(key => {
      localStorage.setItem(`sound${key.charAt(0).toUpperCase() + key.slice(1)}`, temporarySounds[key]);
    });
    onClose();
  };
  ...
  const renderContent = () => {
    switch (view) {
      ...
      case 'audio':
        return (
          <AudioSettings
            sounds={temporarySounds}
            setSounds={setTemporarySounds}
          />
        );
      default:
        return null;
